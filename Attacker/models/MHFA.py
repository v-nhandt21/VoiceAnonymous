...
class MHFA(nn.Module):
    def __init__(self, head_nb=8, inputs_dim=1024, compression_dim=128, outputs_dim=256, layers=25):
        super(MHFA, self).__init__()

        # Define learnable weights for key and value computations across layers
        self.weights_k = nn.Parameter(data=torch.ones(layers), requires_grad=True)
        self.weights_v = nn.Parameter(data=torch.ones(layers), requires_grad=True)
...
#     x = torch.Tensor(6,80,1201)
     x = torch.Tensor(6,1024,600,13)
     model = MHFA()
     o = model(x)

     print(o.shape) # torch.Size([6, 192])