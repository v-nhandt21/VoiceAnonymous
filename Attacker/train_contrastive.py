...
from dataloader_contrastive import get_dataloader
import tqdm, os
...
def train(model, wav_scp_anon, wav_scp_original, save_dir, epochs, cur_epoch):
     ...
     for epoch in tqdm.tqdm(range(cur_epoch, epochs, 1), initial=cur_epoch, total=epochs):
          ...
          for batch_data in tqdm.tqdm(dataloader):
               anon_path_waveform_pos, anon_path_waveform_neg, orig_path_waveform, spk_pos_id = batch_data
               ...
               predict_orig_emb_pos, orig_emb = model(anon_path_waveform_pos, orig_path_waveform)
               predict_orig_emb_neg, orig_emb = model(anon_path_waveform_neg, orig_path_waveform)
